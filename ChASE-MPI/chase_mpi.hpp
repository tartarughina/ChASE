...
//! @brief A derived class of Chase to implement ChASE based on MPI and
//! Dense Linear Algebra (**DLA**) routines.
/*!
  This is a calls derived from the Chase class which plays the
    ...
    class.
    - All the members functions of ChaseMpi, which are the implementation of
  the virtual functions in class Chase, are implemented using the *DLA*
  routines provided by the class ChaseMpiDLAInterface.
    -  The DLA functions in ChaseMpiDLAInterface are also vritual functions,
  which are differently implemented targeting different computing
  architectures (sequential/parallel, CPU/GPU,
  shared-memory/distributed-memory, etc). In the class ChaseMpi, the calling
  of DLA functions are indeed calling their implementations from different
  derived classes. Thus this ChaseMpi class is able to have customized
  implementation for various architectures.
    - For the implementation of the class ChaseMpi targeting
  distributed-memory platforms based on MPI, the setup of MPI environment
  and communication scheme, and the distribution of data (matrix, vectors)
  across MPI nodes are following the ChaseMpiProperties class, the
  distribution of matrix can be either **Block** or **Block-Cyclic** scheme.
    @tparam MF: A class derived from ChaseMpiDLAInterface, which indicates
  the selected implementation of DLA that to be used by ChaseMpi Object.
...
public:
    ...
    /*!
       The private members of this classes are initialized by the parameters
       of this constructor.
       - For the
       variable `N_`, it is initialized by the first parameter of this
       constructor `N`.
       - The variables `nev_` and
       and `nex_` are initialized by the parameters of this constructor
       `nev` and `nex`, respectively.
       - The variable `rank_` is set to be 0 since non MPI is supported. The
       variable `locked_` is initially set to be 0.
       - The variable `config_` is setup by the constructor of ChaseConfig
       which takes the parameters `N`, `nev` and `nex`.
       - The variable `matrices_` is setup directly by the constructor of
       ChaseMpiMatrices which takes the paramter `H`, `ldh`, `N`, `nev`,
       `nex`, `V1`, `ritzv`, `V2` and `resid`.
       ...
       @param nex: Number of eigenvalues augmenting the search space.
       Usually a relatively small fraction of `nev`.
       @param H: a pointer to the memory which stores local part of matrix
       on each MPI rank. If `H` is not provided by the user, it will be
       internally allocated in ChaseMpiMatrices class.
       @param ldh: leading dimension of `H`
       @param V1: a pointer to a rectangular matrix of size `N * (nev+nex)`.
       If `V1` is not provided by the user, it will be internally allocated
       in ChaseMpiMatrices class. After the solving step, the first `nev`
       columns of `V1` are overwritten by the desired Ritz vectors.
       @param ritzv: a pointer to an array to store the computed Ritz
       values. If `ritzv` is not provided by the user, it will be internally
       ...
       eigenpairs. If `resid` is not provided by the user, it will be
       internally allocated in ChaseMpiMatrices class. Its minimal size
       should be `nev+nex`.
    */
    ...
    //! In this case, the buffer for matrix `H` should be allocated
    //! externally and provided by users.
    /*!
       The private members of this classes are initialized by the parameters
       of this constructor.
       - For the
       variable `N_`, `nev_` and `nex_` are initialized by the first
       parameter of this constructor `properties_`.
       - The variable `rank_` is initialized by `MPI_Comm_rank`. The
       variable `locked_` is initially set to be 0.
       - The variable `config_` is setup by the constructor of ChaseConfig
       which takes the parameters `N`, `nev` and `nex`.
       - The variable `matrices_` is constructed by the `create_matrices`
       function defined in ChaseMpiProperties.
       - The variable `dla_` for the distributed-memory is initialized by
       the constructor of ChaseMpiDLA which takes both `properties_` and
       `MF`. In MPI case, the implementation of are split into two classses:
       ...
       for multi-GPUs version.
          - Thus, for this constructor, a combination of ChaseMpiDLA and one
       of ChaseMpiDLABlaslapack and ChaseMpiDLAMultiGPU is required.

       @param properties: an object of ChaseMpiProperties which setups the
       MPI environment and data distribution scheme for ChaseMpi targeting
       distributed-memory systems.
       @param H: a pointer to a local matrix of the distributed
       Symmetric/Hermtitian matrix to be diagonalised. It should be provided
       and allocated by users. The minimal dimension is `mxn`, in which `m`
       and `n` can be obtained through ChaseMpiProperties::get_m() and
    ...
    */
    ...
    //! This member function implements the virtual one declared in Chase
    //! class. \return `config_`: an object of ChaseConfig class which
    //! defines the eigensolver.
    ChaseConfig<T>& GetConfig() override { return config_; }

    //! This member function implements the virtual one declared in Chase
    //! class. Returns the rank of matrix A which is distributed within 2D
    //! MPI grid. \return `N_`: the rank of matrix `A`.
    std::size_t GetN() const override { return N_; }

    //! This member function implements the virtual one declared in Chase
    //! class. Returns Number of desired extremal eigenpairs, which was set
    //! by users. \return `nev_`: Number of desired extremal eigenpairs.
    std::size_t GetNev() override { return nev_; }

    //! This member function implements the virtual one declared in Chase
    //! class. Returns the Increment of the search subspace so that its
    //! total size, which was set by users. \return `nex_`: Increment of the
    //! search subspace.
    std::size_t GetNex() override { return nex_; }

    //! This member function implements the virtual one declared in Chase
    //! class. Returns the array which stores the computed Ritz values.
    //! \return `ritzv_`: an array stores the computed Ritz values.
    Base<T>* GetRitzv() override { return ritzv_; }

    //! This member function implements the virtual one declared in Chase
    //! class. It resets `locked_` to `0` and start to solve a (new)
    //! eigenproblem.
    void Start() override
    {
        locked_ = 0;
    ...
    //! This member function implements the virtual one declared in Chase
    //! class. It indicates the finalisation of solving a single
    //! eigenproblem.
    void End() override { dla_->End(); }

    bool checkSymmetryEasy() override
    {
        is_sym_ = dla_->checkSymmetryEasy();
        return is_sym_;
    }
    ...
    //! This member function implements the virtual one declared in Chase
    //! class. This member function shifts the diagonal of matrix `A` with a
    //! shift value `c`. It is implemented by calling the different
    //! implementation in the derived class of `ChaseMpiDLAInterface`. For
    //! the construction of ChaseMpi with MPI, this function is naturally in
    //! parallel across the MPI nodes, since the shift operation only takes
    //! places on selected local matrices which stores in a distributed
    //! manner.
    ...
    //! This member function implements the virtual one declared in Chase
    //! class. This member function initializes randomly the vectors when
    //! necessary.
    /*!
    ...
    */
    ...
    //! This member function implements the virtual one declared in Chase
    //! class. This member function computes \f$V1 = alpha * H*V2 + beta *
    ...
    */
    ...
    //! construction of the unitary matrix `Q`. After the explicit
    //! construction of Q, its first `locked_` number of vectors are
    //! overwritten by the converged eigenvectors stored previously.
    //!
    ...
    //! possible,
    //! @param fixednev: total number of converged eigenpairs before this
    //! time QR factorization.
    ...
    //! This member function implements the virtual one declared in Chase
    //! class.
    /*! This function performs the Rayleigh-Ritz projection, which projects
       the eigenproblem to be a small one, then solves the small problem and
       reconstructs the eigenvectors.
        @param ritzv: a pointer to the array which stores the computed
       eigenvalues.
        @param block: the number of non-converged eigenpairs, which
       determines the size of small eigenproblem.
    */
    ...
    //! This member function implements the virtual one declared in Chase
    //! class. This member function computes the residuals of unconverged
    //! eigenpairs.
    /*!
         @param ritzv: an array stores the eigenvalues.
         @param resid: a pointer to an array which stores the residual for
       each eigenpairs.
         @param fixednev: number of converged eigenpairs. Thus the number of
       non-converged one which perform this operation of computing residual
       is `nev_ + nex_ + fixednev_`.
    */
    ...
    //! This member function implements the virtual one declared in Chase
    //! class. It swaps the two matrices of vectors used in the Chebyschev
    //! filter
    ...
    //! This member function implements the virtual one declared in Chase
    //! class. It estimates the upper bound of user-interested spectrum by
    //! Lanczos eigensolver
                 ...
                 Base<T>* ritzv, Base<T>* Tau, Base<T>* ritzV) override
    {
        ...
        {
            t_stemr(LAPACK_COL_MAJOR, 'V', 'A', M, d.data() + i * M,
                    e.data() + i * M, ul, ll, vl, vu, &notneeded_m,
                    ritzv + M * i, ritzV, M, M, isuppz.data(), &tryrac);
            for (std::size_t k = 0; k < M; ++k)
            {
                Tau[k + i * M] =
                    std::abs(ritzV[k * M]) * std::abs(ritzV[k * M]);
            }
        }
        ...
        *upperb = std::max(std::abs(ritzv[0]), std::abs(ritzv[M - 1])) +
        ...
        {
            max = std::max(std::abs(ritzv[i * M]),
                           std::abs(ritzv[(i + 1) * M - 1])) +
                  std::abs(real_beta[i]);
    ...
    //! This member function implements the virtual one declared in Chase
    //! class. It locks the `new_converged` eigenvectors, which makes
    ...
    //! This member function implements the virtual one declared in Chase
    //! class. It estimates the spectral distribution of eigenvalues.
    ...
    //! \return `H_`: A pointer to the memory allocated to store (local part
    //! if applicable) of matrix `A`.
    // T* GetMatrixPtr() { return matrices_.get_H(); }

    //! This member function implements the virtual one declared in Chase
    //! class. \return `resid_`: a  pointer to the memory allocated to store
    //! the residual of each computed eigenpair.
    Base<T>* GetResid() override { return resid_; }

    //! This member function return the number of MPI processes used by
    //! ChASE \return the number of MPI ranks in the communicator used by
    //! ChASE
...
private:
    ...
    */
    ...
    /*!
      ...
      - For the constructor of class ChaseMpi with MPI, this variable is
      initialized by `properties`, a pointer to an object of
      ChaseMpiProperties. This variable is private, it can be access by the
      member function GetNev().
    */
    ...
    /*!
      ...
      initialized by `properties`, a pointer to an object of
      ChaseMpiProperties. This variable is private, it can be access by the
      member function GetNex().
    */
    const std::size_t nex_;

    //! The rank of each MPI node with the working MPI communicator
    /*!
      - For the constructor of class ChaseMpi without MPI, this variable is
      `0`.
      - For the constructor of class ChaseMpi with MPI, this variable is
      gotten through MPI function `MPI_Comm_rank`.
    */
    int rank_;
    int nprocs_;

    //! The number of eigenvectors to be locked, which indicates the number
    //! of eigenpairs converged into acceptable tolerance.
    /*!
      - For the constructor of class ChaseMpi without MPI,
    ...
    */
    ...
    //! A pointer to the memory allocated to store the computed eigenvalues.
    //! The values inside are always real since the matrix of eigenproblem
    ...
    /*!
      ...
      pointer to a vector of size `nex_ + nex_`, which are identical on each
      MPI node. It is initalized within the construction of
      ChaseMpiMatrices.
    ...
    */
    ...
    //! A pointer to the memory allocated to store the residual of each
    //! computed eigenpair. The values inside are always real.
    /*!
      - For the constructor of class ChaseMpi without MPI,
      the memory is allocated directly by ChaseMpiMatrices of size `nex_ +
      nex_`.
    ...
    */
    ...
    //! A smart pointer to an object of ChaseMpiProperties class which is
    //! used by ChaseMpi class. This variable only matters for the
    //! constructor of class ChaseMpi with MPI.
    std::unique_ptr<ChaseMpiProperties<T>> properties_;

    //! An object of ChaseMpiMatrices to setup the matrices and vectors used
    //! by ChaseMpi class.
    // ChaseMpiMatrices<T> matrices_;

    //! A smart pointer to an object of ChaseMpiDLAInterface class which is
    //! used by ChaseMpi class.
    /*!
      - For the constructor of class ChaseMpi without MPI,
     this variable is initialized directly by the **template parameter**
     `MF`.
      - For the constructor of class ChaseMpi with MPI, this variable is
     implemented by considering `properties_`  and `MF`. It is initalized
     within the construction of ChaseMpiMatrices.
    */
    std::unique_ptr<ChaseMpiDLAInterface<T>> dla_;

    //! An object of ChaseConfig class which setup all the parameters of
    //! ChASE, these parameters are either provided by users, or using the
    //! default values. This variable is initialized by the constructor of
    //! ChaseConfig class by using `N`, `nev` and `nex`, which are the
    //! parameters of the constructors of ChaseMpi.
